...
- [x] **阶段 1A：Provider Contract v1 历史 Gate**（完成）：验证独立 `wiki_parser` 生命周期、immutable/extra-forbid DTO、固定安全错误、来源/制品 SHA、配额/超时/取消/幂等销毁和完全离线 Fake；旧 Parser 选型文档已删除，不再指导产品实现
- [x] **阶段 1B：冻结 MinerU PDF Parser 架构**：唯一运行时为 MinerU 3.4.5；API 固定 `pipeline|gpu-medium|gpu-high`；每个 Job 单次 attempt、无 Parser 回退；版本化配置不接受 API 任意参数；保存 Parser/version/preset/config/质量/attempt，保持无 Chunk-RAG。设计见 [`docs/design/llm-wiki-mineru-parser.md`](docs/design/llm-wiki-mineru-parser.md)
...
- [x] **阶段 3：实现 PDF/HTML Raw Ingestion**（源码闭环完成；MinerU Contract v2、Raw parse revisions、Fake v2、主应用编排、Worker 合规包、adapter/source Gate 与前端已完成；新 OCI/真实语料 smoke 待复验；设计见 [`docs/design/llm-wiki-raw-ingestion.md`](docs/design/llm-wiki-raw-ingestion.md)）
  ...
  - [x] 升级 `wiki_parser` Contract v2：固定三档 MinerU profile、hash-pinned config、Preflight/Route/Quality、逐页 ParsedDocument、同源 SHA Attempt evidence、规范 Markdown、Artifact v2 与安全错误；每个 Job 恰好一个 attempt，拒绝 fallback。Contract v1 只保留兼容测试，禁止静默混用
  ...
  - [x] 实现完全离线 `FakeMineruParserProvider`：任务创建时核验并冻结原始 PDF；覆盖三档 preset、单次成功/质量拒绝/失败、取消/销毁、hash-pinned config、逐页 Artifact v2 与原始 SHA 不变式
  - [x] 接入 Contract v2 不可信 artifact 导入与现有 Wiki Ingestion/Worker/API：严格交叉核验 receipt/status/manifest/job/source/provider、archive/manifest/逐文件 SHA、规范逐页 Markdown、图片 MIME/魔数与路径/数量/大小配额；`pipeline|gpu-medium|gpu-high` 精确贯穿上传、排队和崩溃恢复，每个 Job 仅保存单次 MinerU attempt/quality/route 证据，artifact 拒绝时保留成功 attempt 但不发布 revision，最终 selected parse revision 与制品/attempt 在同一事务原子提交；支持历史 job/attempt/revision 与指定 revision artifact 浏览，Contract v1 兼容路径保持显式隔离。新增 13 项回归；Wiki 全邻接 `149 passed, 4 skipped`，Backend 全量离线 `4017 passed, 9 skipped, 15 deselected`，Ruff PASS、strict Mypy 160 files / 0 issues
  - [x] 建立独立 MIT Parser Worker 合规包：`workers/wiki_parser_worker` 不进入主 wheel；包含 Worker MIT、MinerU 独立许可提示、notices、精确 source manifest、确定性源码归档、SPDX 2.3 SBOM、自包含 smoke 与 wheel verifier；`/api/about` 和前端可查看与下载
  - [x] 固定 MinerU 3.4.5 并实现 adapter source Gate：Linux/amd64 CPython 3.12，132 包 `uv.lock`、131 包 hash requirements；稳定原件 SHA、PDF 主动内容 gate、三档固定路由、逐页/图片规范化、模型构建期下载、运行期禁止网络
  - [ ] 构建并验证新的断外网 MinerU OCI Worker：CPU pipeline、CUDA medium/high、运行中取消/恢复、SHA/配额/合规门禁和代表性 PDF 实机 smoke；完成前 `runtime_ready=false`
...
- [x] **阶段 7：实现独立 Knowledge 页面并完成退役验收**：新增 `/knowledge` 独立路由与 Pages/Sources/Graph/Changes/Conversations 五视图，支持来源/解析产物、页面/revision、FTS5、图谱、统一 diff 审批及多 Knowledge 对话；删除旧 Library/Document/Chunk 前端，后续 Web-only 审计进一步删除旧 DB/ingestion/indexing/`search_knowledge`/REST 的全部 Backend 运行时与 `rag` extra。当前门禁结果记录于 [`STATUS.md`](STATUS.md)
