...
| **Current phase** | P2-R2-C0 Ingestion Runtime/API Contract — ✅ COMPLETE / FROZEN @ <this commit>（docs-only / contract-only / audit-only；38 项决策已冻结；Schema Amendment **NOT REQUIRED**；R0 §8.1 sync→async 已 minimal refine；4 子阶段 C1/C2/C3/C4 已规划）；下一阶段 P2-R2-C1 ✅ APPROVED TO START（独立启动授权另需用户发起；MVP `generated_at` MUST NOT BE PASSED；详见 [P2_R2_C0_INGESTION_CONTRACT_AUDIT.md](docs/validation/p2-r2/P2_R2_C0_INGESTION_CONTRACT_AUDIT.md)）；R2-D 必查 8-test discrepancy（audit §5.5） |
...
| **R2-C0 Status** | ✅ COMPLETE / FROZEN — Ingestion contract frozen；Schema Amendment NOT REQUIRED（3 non-blocking gaps deferred to R2-D：markdown_sha256 / parser_id / warnings_json）；R0 §8.1 sync→async refined；worker_concurrency=1, queue=32, shutdown_grace=30s。G1 stash `d7240268` 跨 R2-C0 全程未变。详见 [P2_R2_C0_INGESTION_CONTRACT_AUDIT.md](docs/validation/p2-r2/P2_R2_C0_INGESTION_CONTRACT_AUDIT.md) |
