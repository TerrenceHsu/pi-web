...
| **Current phase** | **P2-R3 Heading-aware Chunk + SQLite FTS5 🟡 IN PROGRESS** — R3-A Chunk Contract + Chunker ✅ FROZEN @ `677fe33`（44/44 tests）+ R3-A-R Reliability Closure ✅ FROZEN @ `64b7416`。R3-B1 Schema Migration v1→v2 ✅ COMPLETE / FROZEN（`KNOWLEDGE_SCHEMA_VERSION=2`；knowledge_chunks 扩展 char_count+content_sha256；knowledge_chunks_fts FTS5 virtual table；insert_chunk 兼容 v2；18/18 migration tests + R1 regression 全 PASS；full Backend 3175/0 failed）。R3-B2 Chunk/FTS Store ⛔ APPROVED TO START（独立授权另需）。详见 [p2-r3-b-schema-migration-contract.md](docs/design/p2-r3-b-schema-migration-contract.md)。 |
