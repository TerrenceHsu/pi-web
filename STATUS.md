...
| **Current phase** | P2-R2-C3 Upload / Status / Retry / Markdown API — ✅ COMPLETE / FROZEN @ <this commit>（含 C3-A Upload `72121c5`（UploadService ~700 行 + Upload endpoint + 25 tests）+ C3-B Status/Retry/Markdown `c6a19ec`（3 endpoints + delete guards + 18 tests）+ C3-C Freeze）；4 endpoints under existing Trusted UI + Origin envelope；MAX_PDF_BYTES=25 MB / streaming chunked / SHA incremental / PDF signature / staging→atomic rename / duplicate 409 + status-specific reason / Worker notify；下一阶段 P2-R2-C4 ✅ APPROVED TO START（独立启动授权另需用户发起）；详见 [P2_R2_C3_INGESTION_APIS.md](docs/validation/p2-r2/P2_R2_C3_INGESTION_APIS.md) |
