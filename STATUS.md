...
| **Current phase** | **P2-R3 Bounded Index Worker 🟡 IN PROGRESS** — R3-A ✅ @ 677fe33 / R3-A-R ✅ @ 64b7416 / R3-B ✅ @ a9dd32b / R3-C ✅ @ a4ebf41。R3-D0 Composition Gate ✅（Orchestrator 是唯一 claim owner；Worker read-only discover + process_document；不 double-claim）。R3-D1 IndexingWorkerManager ✅ COMPLETE / FROZEN（indexing_worker.py：bounded single-concurrency / poll=2s / grace=30s / startup recovery wiring（直接 raw SQL cleanup 避免非重入 lock 死锁）/ immediate scan / backlog drain / failure isolation / asyncio.Event wake+stop / no import side effects；19/19 tests PASS；full Backend ×2 = 3318/0 failed consecutive；R3-A/B/C/R2 frozen 模块未触动）。R3-D2 Lifecycle + Delete Guards ⛔ APPROVED TO START（独立授权另需）。 |
