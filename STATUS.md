...
| **Current phase** | **P2-R3 Heading-aware Chunk + SQLite FTS5 🟡 IN PROGRESS** — R3-A Chunk Contract + Chunker ✅ FROZEN @ `677fe33`（44/44 tests）+ R3-A-R Reliability Closure ✅ FROZEN @ `64b7416`。R3-B1 Schema Migration v1→v2 ✅ FROZEN @ `f3401ab`（18/18 tests）。R3-B2 Chunk/FTS Store ✅ COMPLETE / FROZEN（chunk_store.py：replace_document_chunks / delete_document_chunks / rebuild_fts / verify_fts_integrity / search_chunks_fts；safe literal FTS query compiler；BM25 heading-weighted + stable tie-break；ready-only filter；library filter；chunking-only state guard；FTS cleanup 集成到 delete_document_hard / delete_library_hard；66/66 tests PASS；full Backend 3241/0 failed）。R3-B3 Validation Freeze ⛔ APPROVED TO START（独立授权另需）。 |
