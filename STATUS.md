...
| **Current phase** | **P2-R3 Heading-aware Chunk + SQLite FTS5 🟡 IN PROGRESS** — R3-A Chunk Contract + Chunker ✅ COMPLETE / FROZEN（`chunker.py` + `KnowledgeChunk` DTO + `HeadingAwareChunker` pure function + 确定性 `chunk_id` + page/heading/code-fence 解析 + 44/44 单元测试 + 合同文档）。R3-B Schema + SQLite FTS5 ⛔ APPROVED TO START（独立授权另需）。详见 [p2-r3-chunker-contract.md](docs/design/p2-r3-chunker-contract.md)。 |
