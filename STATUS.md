...
| **Current phase** | **P2-R3 Heading-aware Chunk + SQLite FTS5 🟡 IN PROGRESS** — R3-A Chunk Contract + Chunker ✅ COMPLETE / FROZEN @ `677fe33`（`chunker.py` + `KnowledgeChunk` DTO + `HeadingAwareChunker` pure function + 确定性 `chunk_id` + page/heading/code-fence 解析 + 44/44 单元测试 + 合同文档）。R3-A-R Reliability Closure ✅ COMPLETE / FROZEN（causality 排除：含 chunker vs 排除 chunker 同样 Windows sequential-suite flake pattern；chunker 零资源操作；final ×2 = 3157/0 failed）。R3-B Schema + SQLite FTS5 ⛔ APPROVED TO START（独立授权另需）。详见 [p2-r3-chunker-contract.md](docs/design/p2-r3-chunker-contract.md) + [P2_R3_A_R_RELIABILITY_CLOSURE.md](docs/validation/p2-r3/P2_R3_A_R_RELIABILITY_CLOSURE.md)。 |
