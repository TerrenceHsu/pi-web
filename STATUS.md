...
| **Current phase** | P2-R1 Library Foundation — ✅ COMPLETE / FROZEN @ `3bca5fd`（5 表 schema + KnowledgeFileStore + Service + Library CRUD + Session Binding API + 0 PDF 依赖；等待用户授权 merge / tag / push）；下一阶段 P2-R2-0 PDF Parser License Gate 🟡 CONDITIONALLY APPROVED（R2 编码 BLOCKED UNTIL D6 marker AGPL 兼容性确认） |
