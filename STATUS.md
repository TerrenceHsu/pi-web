...
| **Current phase** | P2-R2-C2 Bounded Worker + Recovery — ✅ COMPLETE / FROZEN @ <this commit>（含 C2-A Worker `5aea74b`（~700 行 + 30 tests）+ C2-B Lifespan `115136a`（web/app.py + web/state.py 接线 + 7 tests）+ C2-C Freeze）；IngestionWorkerManager 单 worker / 32-capacity queue / 30s poll / 30s shutdown grace；startup recovery（running Job + extracting/normalizing Document → failed + ingestion_interrupted）；conditional UPDATE 防 shutdown race；FastAPI lifespan wiring（manager.stop() before KnowledgeStore.close()）；下一阶段 P2-R2-C3 ✅ APPROVED TO START（独立启动授权另需用户发起）；详见 [P2_R2_C2_WORKER_RECOVERY.md](docs/validation/p2-r2/P2_R2_C2_WORKER_RECOVERY.md) |
...
| **R2-C2 Status** | ✅ COMPLETE / FROZEN — IngestionWorkerManager + FastAPI lifespan wiring。37/37 targeted tests；3023 backend passed（delta 37 = C2 targeted）；0 functional regression；frontend 267/267；ruff clean。详见 [P2_R2_C2_WORKER_RECOVERY.md](docs/validation/p2-r2/P2_R2_C2_WORKER_RECOVERY.md) |
