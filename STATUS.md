...
| **Current phase** | **P2-R3 Indexing Runtime 🟡 IN PROGRESS** — R3-A ✅ @ `677fe33` + R3-A-R ✅ @ `64b7416` + R3-B Schema+FTS5 ✅ @ `a9dd32b`。R3-C1 IndexingStore ✅ COMPLETE / FROZEN（indexing_store.py：claim_document / claim_next_normalizing_document / mark_indexing / mark_ready / mark_failed / recover_interrupted_indexing；SQLite conditional write atomic claim；不引入 indexing_jobs 表；schema diff=0；37/37 tests PASS；full Backend 3278/0 failed；R3-A/B/R2 frozen 模块未触动）。R3-C2 IndexingOrchestrator ⛔ APPROVED TO START（独立授权另需）。 |
