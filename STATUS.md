...
| **Current phase** | **P2-R3 Indexing Runtime 🟡 IN PROGRESS** — R3-A ✅ @ `677fe33` + R3-A-R ✅ @ `64b7416` + R3-B Schema+FTS5 ✅ @ `a9dd32b`。R3-C1 IndexingStore ✅ @ `dbf5593`。R3-C2 IndexingOrchestrator ✅ COMPLETE / FROZEN（indexing_orchestrator.py：process_document 显式调用；T1 claim→chunking；T2 read document.md + source_sha256 完整性；T2b HeadingAwareChunker（含 frontmatter+page_count 验证）；T3 ChunkStore.replace_document_chunks；T4 mark_indexing；T5 FTS integrity gate；T6 mark_ready；IndexingResult DTO；safe error taxonomy 8 codes；failure compensation 自动清理 chunks+FTS；21/21 tests PASS；full Backend 3299/3/14/0 failed @ Run #3；R3-A/B/C1/R2 frozen 模块未触动）。R3-C3 Validation Freeze ⛔ APPROVED TO START（独立授权另需）。 |
