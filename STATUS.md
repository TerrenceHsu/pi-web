...
| **Current phase** | P2-R2-C1 Ingestion Store + Orchestrator — ✅ COMPLETE / FROZEN @ <this commit>（含 C0 Archive Correction `0a8f554`（R2-C terminal = `normalizing`）+ C1-A Store `015dc45`（463 行 + 38 tests）+ C1-B Orchestrator `cde0e0b`（722 行 + 20 tests）+ C1-C Freeze）；R2-C terminal = `normalizing`（per C0 §35.5 post-freeze correction；状态机 `normalizing → ready` 不允许，R2-C 绝不调用 `transition_document_status(doc_id, 'ready')`）；下一阶段 P2-R2-C2 ✅ APPROVED TO START（独立启动授权另需用户发起）；详见 [P2_R2_C1_INGESTION_ORCHESTRATOR.md](docs/validation/p2-r2/P2_R2_C1_INGESTION_ORCHESTRATOR.md) |
| **R2-B Archive Closure** | ✅ COMPLETE — User decision: **A — RATIFIED** @ 2026-08-03（c2436c7）；Amendment 2 APPROVED；8-test discrepancy = KNOWN NON-BLOCKING，MUST RECONCILE IN R2-D。详见 [P2_R2_B_AMENDMENT2_AUTHORIZATION_AUDIT.md §7](docs/validation/p2-r2/P2_R2_B_AMENDMENT2_AUTHORIZATION_AUDIT.md) |
...
| **R2-C1 Status** | ✅ COMPLETE / FROZEN — IngestionStore（atomic claim / retry / recovery）+ IngestionOrchestrator（Parser+Quality+Builder+Persistence composition）。58/58 targeted tests；2986 backend passed（delta 66；8-test discrepancy 继续 handoff to R2-D）；0 functional regression；frontend 267/267；ruff clean。详见 [P2_R2_C1_INGESTION_ORCHESTRATOR.md](docs/validation/p2-r2/P2_R2_C1_INGESTION_ORCHESTRATOR.md) |
| **R3 Retrieval Scope** | 🔄 REFINED @ 2026-08-03 — pure FTS5 BM25（无向量）；D1（embedding）/ D2（hybrid）/ D3（re-embedding）**永久不做**（per 用户决策"PDF→MD 即可，不再向量化"；R0 §3 推迟项协议直接冻结，**不创建 Amendment 3**）。chunk + FTS5 + search_knowledge tool 保留；evidence 含 page marker 引用（R2-B 已实现）。详见 [decisions-log §3.1](docs/design/p2-r0-decisions-log.md)。R2-C0 / R2-C1-C4 不受影响（R2-C 范围仅到 markdown 持久化）。 |
