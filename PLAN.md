...
> 当前代码位于 `0.0.29` 发布基线之后：三类意图路由与 Planner–Executor–Verifier Plan Mode 已完成；与上游 pi 的逐模块对齐已从第 1 项 `ai` 开始，首轮核心消息/Provider/Usage/重试契约已经补齐。Session Workspace、自动 Memory、Managed Coding Sandbox、LLM Wiki、Approval、Context Budget/Compaction 等既有主线保持有效。
>
> 当前后续顺序是继续逐项审查 `agent`、`coding-agent`、`session-backends/sqlite-node`、`telemetry`。实施事实和最新门禁数字见 [STATUS.md](STATUS.md)，AI 对齐证据见 [pi-ai parity report](docs/validation/pi-ai-parity-2026-08-31.md)，后续优先级与明确延期项见 [ROADMAP.md](ROADMAP.md)。
