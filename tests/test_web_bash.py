...
class LocalDouble(FakeSandboxBackend):
    ...
    async def execute_bash(self, handle, command, *, script, signal=None, on_output=None):
        ...
        return SandboxCommandResult(
            command_id=command.command_id, exit_code=self.exit_code, stdout="中文 output\n",
            stderr="error\n" if self.exit_code else "", started_at_ms=1, finished_at_ms=2,
        )
...
def make_client(
    tmp_path: Path, *, script=SCRIPT, factory=None, image=None, scripts=None, policy=None,
):
    backend = LocalDouble()
    model = FakeClient(scripts or [
        _plan_role_call("bash-call", "run_bash", {
            "script": script, "cwd": ".", "timeout_seconds": 60, "reason": "Inspect copy",
        }),
    ...
    app = create_app(
        harness, db_path=tmp_path / "db.sqlite", uploads_dir=tmp_path / "uploads",
        credential_secret_backend="memory", credential_extra_hosts=("testserver",),
        enable_trusted_host=True, enable_intent_routing=True, enable_plan_mode=True,
        local_docker_config=LocalDockerExecutionConfig(
            enabled=True, image_id=image or "sha256:" + "f" * 64,
        ),
...
def select_bash(client):
    ...
    assert client.put(f"/api/workspaces/{sid}/execution", json={
        "backend": "local_docker", "expected_revision": 0,
    }).status_code == 200
    assert client.put(f"/api/workspaces/{sid}/extensions", json={
        "mcp_server_names": [], "skill_names": [], "tool_names": ["run_bash"],
    }).status_code == 200
    return sid


def start_bash(client, sid, **fields):
    value = client.post("/api/prompt/async", json={
        "text": "run_bash 检查副本", "session_id": sid, **fields,
    })
...
def seed(client, rid, backend):
    ...
    entries = tuple(SandboxFileEntry(path=e.path, size=e.size, sha256=e.sha256)
...
@pytest.mark.parametrize("text,mode,route", [
...
@pytest.mark.parametrize("args", [
    {"script": "true", "approved": True}, {"script": "true", "cwd": "../"},
    {"script": "x" * 16385}, {"script": "bad\0script"},
    {"script": "true", "timeout_seconds": 301}, {"script": "true", "cwd": "absent"},
])
def test_invalid_or_nonexistent_scope_cannot_create_runtime(tmp_path, args):
    test_client, backend = make_client(tmp_path, scripts=[
        _plan_role_call("bad", "run_bash", args), [DoneEvent(stop_reason="stop")],
    ])
    with test_client as client:
        sid = select_bash(client)
        rid = start_bash(client, sid)
...
def test_real_standalone_bash_reads_upload_reports_changes_and_discards_copy(tmp_path):
    ...
    def factory(config):
        return LocalDockerSandboxBackend(config=config, transport=DockerCLITransport(
            executable=Path(os.environ["PI_TEST_DOCKER_EXE"]), config_directory=config_root,
