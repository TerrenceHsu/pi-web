...
| **R2-B Canonical Markdown Builder** | ✅ FROZEN | R2-A | P2-R0 Amendment 2（frontmatter 字段集 + needs_ocr 阈值精化）+ pdf_quality 评估 + heading 启发式 + Canonical MD frontmatter + 固定 page marker + KnowledgeFileStore 原子写入（**不接 HTTP API**） | [P2_R2_B_CANONICAL_MARKDOWN.md](docs/validation/p2-r2/P2_R2_B_CANONICAL_MARKDOWN.md) 60/60 exit gate PASS / 2920 backend + 267 frontend / 0 regression |
| **R2-C Ingestion Worker + Upload/Retry API** | ✅ APPROVED TO START | R2-B | 上传 endpoint + Ingestion Job + worker + 状态转移 + retry + Document API 接线 | 30s 超时分支 / Job 状态全路径 / upload smoke / retry 幂等 |
