...
| **R2-B Canonical Markdown Builder** | 🟡 COMPLETE — Archive Closure REQUIRED | R2-A | P2-R0 Amendment 2（用户 AskUserQuestion 显式授权；frontmatter 字段集 + needs_ocr 阈值精化）+ pdf_quality 评估 + heading 启发式 + Canonical MD frontmatter + 固定 page marker + KnowledgeFileStore 原子写入（**不接 HTTP API**） | [P2_R2_B_CANONICAL_MARKDOWN.md](docs/validation/p2-r2/P2_R2_B_CANONICAL_MARKDOWN.md) 60/60 exit gate PASS / [P2_R2_B_AMENDMENT2_AUTHORIZATION_AUDIT.md](docs/validation/p2-r2/P2_R2_B_AMENDMENT2_AUTHORIZATION_AUDIT.md) 待用户追认 |
| **R2-C Ingestion Worker + Upload/Retry API** | ⛔ BLOCKED BY R2-B Archive Closure | R2-B | 上传 endpoint + Ingestion Job + worker + 状态转移 + retry + Document API 接线（**MVP 不传 generated_at** per audit §6） | 30s 超时分支 / Job 状态全路径 / upload smoke / retry 幂等 |
| **R2-D Integration Validation + Freeze** | ⛔ BLOCKED BY R2-C | R2-C | 整合 R2-A/B/C 全链路 + 集成测试 + 最终 freeze | E2E upload→ingest→markdown 全链路 / 0 regression / tag candidate |
