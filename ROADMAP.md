...
| **R2-B Canonical Markdown Builder** | ✅ APPROVED TO START | R2-A | 数字 PDF 文本质量判断 + needs_ocr 判定 + heading 启发式 + Canonical MD frontmatter + 固定 page marker + Markdown 原子写入（**不接 HTTP API**） | 数字 PDF→MD smoke / 扫描 PDF→needs_ocr 终态 / page marker 稳定 / frontmatter 字段完整 |
| **R2-C Ingestion Worker + Upload/Retry API** | ⛔ BLOCKED BY R2-B | R2-B | 上传 endpoint + Ingestion Job + worker + 状态转移 + retry + Document API 接线 | 30s 超时分支 / Job 状态全路径 / upload smoke / retry 幂等 |
...
| **R2 (整体) Ingestion Pipeline** | 🟡 IN PROGRESS | R2-0 | R2-A 完成；R2-B/C/D 待启动 | — |
