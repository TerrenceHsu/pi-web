...
| **R5 Web API + UI** | 🟡 IN PROGRESS (R5-A done) | R4 | library / document / search REST + 前端管理面板 | E2E upload→ingest→search 全链路 |
| **R5-A Web API + UI Contract Audit** | ✅ COMPLETE / FROZEN @ `<this commit>` | R4 | docs-only audit + freeze | [P2_R5_A_WEB_CONTRACT.md](docs/validation/p2-r5/P2_R5_A_WEB_CONTRACT.md) 16/16 exit gate PASS / 14 existing REST reused / 1 new Search REST / production diff=0 |
| **R5-B Knowledge REST API** | ⛔ BLOCKED BY R5-A → APPROVED TO START | R5-A | Search REST + Library/Document gap fill | targeted tests §19.1 / 0 duplicate API / schema diff=0 |
| **R5-C Frontend Knowledge Manager** | ⛔ BLOCKED BY R5-B | R5-B | KnowledgeManagerModal + Library/Document/Upload/Search/Binding UI | vitest + typecheck + lint + build PASS |
| **R5-D Final E2E Freeze** | ⛔ BLOCKED BY R5-C | R5-C | Upload→Ingest→Search→Binding E2E + Agent continuity | Full Backend ×2 = 0 failed / Browser E2E (or integration tests in this copy) PASS |
