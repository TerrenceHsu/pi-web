...
| **R3-A Chunk Contract + Chunker** | ✅ COMPLETE / FROZEN @ `<this commit>` | R2-D-B | `chunker.py` + `KnowledgeChunk` DTO + `HeadingAwareChunker` pure function；deterministic chunk_id；heading/page/frontmatter/code-fence 解析；44/44 unit tests | [p2-r3-chunker-contract.md](docs/design/p2-r3-chunker-contract.md) CHUNK_TARGET=1200 / MAX=1800 / OVERLAP=160 / 0 production dep / 0 R2 module touched |
| **R3-B Schema + SQLite FTS5** | ⛔ APPROVED TO START（独立授权另需） | R3-A | schema v1→v2 migration；`knowledge_chunks_fts` virtual table；FTS sync triggers；safe literal FTS query compiler；BM25 ranking + heading weight | FTS5 capability probe 已 PASS（unicode61 remove_diacritics 2 / bm25 / 中文 / rollback）|
