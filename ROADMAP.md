...
| **R2-C Ingestion Worker + Upload/Retry API** | ✅ APPROVED TO START（独立启动授权另需用户发起） | R2-B | 上传 endpoint + Ingestion Job + worker + 状态转移 + retry + Document API 接线（**MVP `generated_at` MUST NOT BE PASSED** per audit §6） | 30s 超时分支 / Job 状态全路径 / upload smoke / retry 幂等 |
