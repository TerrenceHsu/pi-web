...
| **R2-C1 Store Extensions + Orchestrator** | ✅ FROZEN @ `cde0e0b` + freeze commit | R2-C0 | IngestionStore（atomic claim / retry / recovery）+ IngestionOrchestrator（Parser+Quality+Builder+Persistence composition）；terminal = `normalizing` (per C0 §35.5 correction) | [P2_R2_C1_INGESTION_ORCHESTRATOR.md](docs/validation/p2-r2/P2_R2_C1_INGESTION_ORCHESTRATOR.md) 58/58 exit gate PASS / 2986 backend (delta 66; 8-test discrepancy non-blocking) / 267 frontend / 0 regression |
| **R2-C2 Bounded Worker + Recovery** | ⛔ BLOCKED BY C1 | R2-C1 | WorkerManager + lifespan 接线 + startup recovery + graceful shutdown | 30s grace / running→failed on crash / queue 32 / concurrency 1 |
| **R2-C3 Upload / Status / Retry / Markdown API** | ⛔ BLOCKED BY C2 | R2-C2 | 4 endpoints + Body limit + Trusted UI + DTO + API 测试 | 201 async / 409 duplicate / 413 too large / 415 invalid signature |
