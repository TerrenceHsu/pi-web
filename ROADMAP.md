...
| **R3 Retrieval** | ✅ APPROVED TO START（独立授权另需） | R2 | `search_knowledge` tool + **pure FTS5 BM25**（无向量；D1/D2/D3 DECLINED @ 2026-08-03）+ heading-aware chunker + top_k 排序 + 引用 evidence（chunk.page_start / page_end 即 page marker 引用，per R2-B `<!-- page:N -->`） | query 召回 / tool 不暴露 session_id（AST 校验） / 0 vector dependency |
